...
k8s_status_map = util.k8s_status_map
...
def monitor_running_job():
    ...
    # 进行下一次定时任务
    # logger.info("monitor_running_job: status is monitoring...")
    ...
    for item in info:
        ...
        if int(minutes) >= 1:  # 如果大于1分钟则开始监控
            ...
            status_id = k8s_status_map[status]

            if status_id == 200:
                update_sql = "update airpipline_trainjobtab set end_time = 'None', status_id = 200 where id = {0}".format(train_id)
                _, _ = DB.update(update_sql)
            elif (("flag" in params['automl'].keys()) and (params['automl']['flag'] == True)): #automl
                pass
            else:
                end_time = now_time
                update_sql = "update airpipline_trainjobtab set end_time = '{0}', status_id = {1} where id = {2}".format(
                    end_time, status_id, train_id)
                _, _ = DB.update(update_sql)



    monitor_running_job_runner = Timer(get_config("status_monitor", "monitor_running_job"), monitor_running_job)
    ...
    for id, user_id, status_id, create_time in info:
        ...
        if status_id == 80:
            ...
            if int(minutes) >= 20: # 如果大于20分钟则删除
                ...
                # 删除文件
                if os.path.exists(os.path.join(get_config('path', 'airpipline_path'), "external", str(user_id), "train", str(id))):
                    util.remove_dir(
                        os.path.join(get_config('path', 'airpipline_path'), "external", str(user_id), "train",
                                     str(id)))
                ...
                flag, info = k8s_ctl.k8s_delete(
                    token=private_key,
                    pod_name=str(id) + "_" + info[1],
                    task_id=info[1]
                )

                logger.info("monitor_delete_invaild_train_task: delete train id {}".format(id))

    monitor_delete_invaild_train_task_runner = Timer(get_config("status_monitor", "monitor_delete_invaild_train_task"), monitor_delete_invaild_train_task)
