...
def sampleset_from_id_to_path(token, dataset_id, class_type='IMAGE'):
    ...
    response = requests.get(url=get_config('sample_set', 'id2path_url').format(get_config('IP', 'SAMPLE_SET'), dataset_id, class_type),
                            headers={"token": token})
    try:
        logger.info("sampleset_from_id_to_path: {}".format(response))
        datapath = os.path.join(get_config('path', 'data_path'), json.loads(response.text)['data'][0]['path'])
...
def sampleset_from_ids_to_infos(token, dataset_ids):
    ...
    response = requests.get(url=get_config('sample_set', 'ids2infos').format(get_config('IP', 'SAMPLE_SET'), ",".join(dataset_ids)),
                            headers={"token": token})
