...
def k8s_create(token, pod_name, image_id, image_name, lables, volumeMounts=None, port_map=None, params=None, train_cmd=None):
    ...
    k8s_instance.service_data['port'] = []
    if port_map != None:
        for port_idx, port_info in enumerate(port_map):
            k8s_instance.service_data['port'].append(
                {
                    "node_port": port_info['src'],
                    "code_port": port_info['dst']
                }
            )
    ...
    k8s_instance.task_info['working_type'] = 1
